...
warmth_orient <- kruskal.test(Animal ~ Breakfast, data = df)
...
library(rstatix)
warmth_medium_effect <- source_year_means %>%
  wilcox_effsize(mean_warmth ~ Medium)
comp_medium_effect <- source_year_means %>%
  wilcox_effsize(mean_competence ~ Medium)
# Returns r (effect size)
warmth_orient_effect <- source_year_means %>%
  kruskal_effsize(mean_warmth ~ Political_Orientation)
comp_orient_effect <- source_year_means %>%
  kruskal_effsize(mean_competence ~ Political_Orientation)
warmth_source_effect <- source_year_means %>%
  kruskal_effsize(mean_warmth ~ Source)
comp_source_effect <- source_year_means %>%
  kruskal_effsize(mean_competence ~ Source)
# Returns eta-squared
...
# Add a header row for each dimension
...
ft <- flextable(final_table) %>%
  ...
  merge_v(j = "Dimension") %>%  # vertically merge dimension column
  set_header_labels(
  ...
  add_footer_lines("Note: Per APA (2026) statistical significant, ns p = ≥ 0.05 (not significant), *p = ≤ 0.05 (significant), **p = ≤ 0.01 (highly significant), ***p = < 0.001 (extremely significant). For the effect size (ES), Per Cohen (1988) for the Mann-Whitney U test, r = .10 (small ES), r = .30 (medium ES), r = .50 (large ES). Per Tomczak & Tomczak (2014) for the Kruskal-Wallis test, η² = .01 (small ES), η² = .06 (medium ES), η² = .14 (large ES). Per Cohen (1988) for Spearman's rho test, ρ ≥ .10 (small ES), ρ ≥ .30 (medium ES), ρ ≥ .50 (large ES). The number of tests at the source/year equate to (n = 117) for this study.") %>%
  autofit()
...
# Show source-level warmth distributions
source_year_means %>%
  group_by(Source) %>%
  summarise(
    n = n(),
